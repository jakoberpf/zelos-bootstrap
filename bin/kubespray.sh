...
docker run --rm -it \
  ...
  quay.io/kubespray/kubespray:v2.18.1 bash -c "ansible-playbook -vvv --become --become-user=root -i /inventory/inventory.ini -b --private-key /root/.ssh/id_rsa cluster.yml"
